...
class UserRepository {
  ...
  Future<void> updateUserData(User user) async {
    await usersReference.doc(user.id).set(_mapUser(user));
  }
  ...
  Future<User> getUserData(String uid) async {
    ...
    if (snapshot.exists) {
      ...
      return User(
        ...
        name: '',
        photoURL:
            'https://firebasestorage.googleapis.com/v0/b/dos-hermanos.appspot.com/o/Profile%20Images%2Fdefault_profile_pic.jpg?alt=media&token=33961555-a0d8-48aa-9a48-124c1e397aeb',
        profilePhoto: Image.asset('assets/default_profile_pic.jpg'),
      );
    }
  }

  Future<String> putProfileImage(
      {required File image, required String name}) async {
    final firebase_storage.Reference profileImage =
        firebase_storage.FirebaseStorage.instance.ref().child('Profile Images');

    final firebase_storage.UploadTask uploadTask =
        profileImage.child(name + ".jpg").putFile(image);
  ...
  Future<User> fromSnapshot(DocumentSnapshot snapshot) async {
    ...
    return User(
        ...
        profilePhoto: await getProfileImage(
            data['photoURL'] ?? Image.asset('assets/default_profile_pic.jpg')));
  }
}
