...
class DrawerBloc extends Bloc<DrawerEvent, DrawerState> {
  ...
  DrawerBloc(
      {required this.authenticationRepository, required this.localDataBase})
      : super(DrawerInitial(localDataBase: localDataBase)) {
    on<ChangeProfilePhoto>((event, emit) async {
      emit(LoadingDrawer(localDataBase: localDataBase));
      try {
        authenticationRepository.user.photoURL =
            await authenticationRepository.userRepository.putProfileImage(
                image: event.newphotoFile,
                name: authenticationRepository.user.name);
        authenticationRepository.user.profilePhoto = event.newphoto;
        await authenticationRepository.userRepository
            .updateUserData(authenticationRepository.user);
    ...
    on<ChangeLocation>((event, emit) async {
      emit(LoadingDrawer(localDataBase: localDataBase));
      try {
        authenticationRepository.user.location =
            Location.fromName(event.locationName);
        await authenticationRepository.userRepository
            .updateUserData(authenticationRepository.user);
        emit(DrawerLoaded(localDataBase: localDataBase));
      } catch (e) {
    ...
    on<LoadDrivers>((event, emit) async {
      ...
      try {
        Response? token =
            await _driverProvider.callApi(endpoint: 'oauth/token', parameters: {
          "grant_type": "client_credentials",
          "client_id": DHApiKeys().clientID,
          "client_secret": DHApiKeys().clientStcret
        });

        Response? driversJson = await _driverProvider.callApi(
            endpoint: 'choferesapp/list',
            parameters: {"ACCESS_TOKEN": token!.body});

        DriverRepository driversrepo =
            DriverRepository.fromJson(json.decode(driversJson!.body));

        for (var i = 0; i < driversrepo.drivers.length; i++) {
          if (driversrepo.drivers[i].active) {
            Response? paramResponse = await _driverProvider.callApi(
                endpoint: 'choferesapp/' + driversrepo.drivers[i].code,
                parameters: {"ACCESS_TOKEN": token.body});
            if (paramResponse!.statusCode == 200) {
              ...
              driversrepo.drivers[i].chasisPatent =
                  parameters["PatenteAcoplado"] ?? '';
              driversrepo.drivers[i].company =
                  parameters["Transportista"] ?? '';
              print(i.toString());
            } else {
            ...
            emit(LoadingDrivers(
                localDataBase: localDataBase,
                progress: i / driversrepo.drivers.length));
          }
        }

        localDataBase.driversDB = driversrepo.drivers;
        await DataBaseFileRoutines()
            .writeDataBase(databaseToJson(localDataBase));
